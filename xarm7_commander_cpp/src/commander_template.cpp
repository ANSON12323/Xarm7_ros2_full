...
#include <xarm7_interfaces/msg/pose_command.hpp>

using MoveGroupInterface = moveit::planning_interface::MoveGroupInterface;
using Bool = example_interfaces::msg::Bool;
using FloatArray = std_msgs::msg::Float64MultiArray;
using PoseCmd = xarm7_interfaces::msg::PoseCommand;
...
private:
    ...
    void jointCmdCallback(const FloatArray &msg)
    {
        ...
        RCLCPP_INFO(node_->get_logger(), "Received valid joint command with %zu joints.", joints.size());

        if (joints.size() == 7) {
            RCLCPP_INFO(node_->get_logger(), "Received valid joint commands, Planning motion...");

            goToJointTarget(joints);
        }
        else{
            std::cout<<"Values less than 7";
        }
    }
